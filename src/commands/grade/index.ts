...
function generateQualityMd(scores: DomainScore[], history: HistoryEntry[]): string {
  ...
  for (const score of currentScores) {
    ...
    for (const dim of dimensions) {
      ...
      if (currentIdx < prevIdx) {
        trends.push(`${score.domain}/${dim}: ${prevGrade} -> ${currentGrade} (improved)`);
      } else if (currentIdx > prevIdx) {
        trends.push(`${score.domain}/${dim}: ${prevGrade} -> ${currentGrade} (degraded)`);
      }
    }
